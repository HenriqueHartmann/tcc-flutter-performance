...
void main() {
  ...
  group('BLoC', () {
    testWidgets('Choose BLoC in the index', (WidgetTester tester) async {
      ...
      // Choose the bloc option by tapping the the libSelectionItem Key from the parent blocSelection Key
      await tester.tap(find.ancestor(
          of: find.byKey(KeysConstant.getLibSelectionItemKey()),
          matching: find.byKey(KeysConstant.getBlocSelectionKey())));

      // Wait once again for the content to be loaded
      await tester.pumpAndSettle();

      // Expect to be in the set state page
      expect(find.byKey(KeysConstant.getBlocPageKey()), findsOneWidget);
    });

    testWidgets('Change all card colors in BLoC page',
      ...
      final MainBlocState state = tester.state(find.byType(MainBloc));

      // Expect the switch of all card background color value to be true
      expect(state.cardBloc.state.getSwitchAllColors(), true);
    });

    testWidgets('Change all title colors in BLoC page',
        (WidgetTester tester) async {
      // Initiate an instance of the App
      app.main();
      ...
      expect(state.titleBloc.state.getSwitchAllColors(), true);
    });
    ...
    testWidgets('Change all title font size in SetState page',
      ...
      expect(state.titleBloc.state.getSwitchAllFontSize(), true);
    });

    testWidgets('Change all description colors in BLoC page',
        (WidgetTester tester) async {
      // Initiate an instance of the App
      app.main();
      ...
      expect(state.descriptionBloc.state.getSwitchAllColors(), true);
    });
    ...
    testWidgets('Change all description font sizes in BLoC page',
      ...
      expect(state.descriptionBloc.state.getSwitchAllFontSize(), true);
    });
  });
}
